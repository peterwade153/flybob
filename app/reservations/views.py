...
class ReservationsView(MethodView):
    ...
    def post(self, current_user):
        ...
        if seats_booked > flight.capacity:
            return (
                jsonify(
                    {
                        "message": "Current seating capacity is "
                        + str(flight.capacity),
                        "status": "Failed",
                    }
                ),
    ...
    def get(self, current_user):
        ...
        if flight and booked_on:
            seats_booked = 0
            booked_flights = Reservation.query.join(Flight).filter(
                func.date(Reservation.booked_on) == booked_on,
                Flight.name == flight,
            )
...
class ReservationGetUpdateView(MethodView):
    ...
    def put(self, current_user, id):
        ...
        if not reservation:
            return (
                jsonify(
                    {"message": "Reservation not found", "status": "Failed"}
                ),
                404,
            )
        ...
        for attr, value in data.items():
            ...
            reservation.save()
        return (
            jsonify({"message": "Reservation updated", "status": "Success"}),
            200,
        )
