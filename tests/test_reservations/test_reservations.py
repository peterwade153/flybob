...
class ReservationsTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.reservation_data = {"flight_id": 1, "seats_booked": 2}
    ...
    def test_add_reservations_with_no_seats_available(self):
        ...
        token = json.loads(res.data.decode())["access_token"]
        book_data = {"flight_id": 1, "seats_booked": 244}
        result = self.app.post(
            "/api/v1/reservations",
    ...
    def test_add_reservations_for_non_existant_flight(self):
        ...
        token = json.loads(res.data.decode())["access_token"]
        non_existant_data = {"flight_id": 6, "seats_booked": 2}
        result = self.app.post(
            "/api/v1/reservations",
            data=json.dumps(non_existant_data),
            headers={
                "Content-Type": "application/json",
                "Authorization": token,
            },
    ...
    def test_add_reservations_for_with_wrong_data(self):
        ...
        token = json.loads(res.data.decode())["access_token"]
        invalid_data = {"flight_iddfty": 6, "seats_bookedjhbjhb": 2}
        result = self.app.post(
            "/api/v1/reservations",
    ...
    def test_update_a_non_existant_reservations(self):
        ...
        token = json.loads(res.data.decode())["access_token"]
        update_data = {"seats_booked": 2, "status": "cancelled"}
        result = self.app.put(
            "/api/v1/reservations/1",
    ...
    def test_update_a_reservations_with_invalid_data(self):
        ...
        update_data = {"flight_booked": 2, "status": "cancelled"}
        result = self.app.put(
            "/api/v1/reservations/1",
            data=json.dumps(update_data),
            headers={
    ...
    def test_get_reservations_made_on_specific_day(self):
        ...
        result = self.app.get(
            ...
            data={"flight": "KQ-FTK123", "booked_on": "2019-04-25"},
        )
        self.assertEqual(result.status_code, 200)
