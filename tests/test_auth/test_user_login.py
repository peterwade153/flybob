...
class AuthLoginTestCase(unittest.TestCase):
    ...
    def register_user(self, username='john123', email='john123@john.com', password='john1234556'):
        user_data = {
            'username':username,
            'email':email,
            'password':password
        }
        return self.app.post('/api/v1/auth/register', data=json.dumps(user_data),
    ...
    def tests_unregistered_user_login(self):
        reg = self.app.post('/api/v1/auth/login', data=json.dumps(self.user_data), 
                            content_type='application/json')
